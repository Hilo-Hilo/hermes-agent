...
A test that reads a source file's text is testing _the shape of the
source code_, not its behavior. This is a hard antipattern, banned outright.
...
test("backend spawn hides the Windows console", () => {
  assert.match(
    source,
    /spawn\(\s*backend\.command,\s*backend\.args[\s\S]{0,300}hiddenWindowsChildOptions/,
  );
});
```
...
test("windowsHide defaults to true on Windows, is left alone elsewhere", () => {
  ...
  assert.equal(
    hiddenWindowsChildOptions({ windowsHide: false }, true).windowsHide,
    false,
  );
});
```
